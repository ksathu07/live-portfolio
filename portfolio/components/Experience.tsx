    ...
    >
      <div className="group relative">
        ...
        <div className="absolute -left-[31px] top-1.5 h-3 w-3">
          ...
          <span className="absolute inset-0 rounded-full bg-gradient-to-r from-gold-300 to-gold-500 blur-sm opacity-60 group-hover:opacity-100 transition-opacity" />
          <span className="absolute inset-[-4px] rounded-full border border-gold-400/0 group-hover:border-gold-400/50 scale-50 group-hover:scale-100 transition-all duration-500" />
        </div>

        <h3 className="font-display font-semibold text-white group-hover:text-gradient transition-all duration-300">
          {item.role}
        </h3>
        <p className="text-sm font-medium text-gold-300 group-hover:text-gold-200 transition-colors">
          {item.organization}
        </p>
          ...
          <ul className="mt-2 space-y-1.5 text-sm leading-relaxed text-zinc-400">
            {(item.bullets ?? []).map((b, j) => (
              <li key={j} className="flex gap-2 group-hover:translate-x-0.5 transition-transform duration-300" style={{ transitionDelay: `${j * 50}ms` }}>
                <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gold-400/60 group-hover:bg-gold-400 transition-colors" />
...
function EducationCard({ item, index }: { item: (typeof profile.education)[number]; index: number }) {
  ...
  return (
    <div
      ...
      style={{ transitionDelay: `${index * 120}ms` }}
    >
      <div className="glass rounded-2xl p-6 transition-all duration-500 group-hover:-translate-y-1 group-hover:border-gold-400/30 group-hover:bg-white/[0.06] group-hover:shadow-[0_20px_60px_-15px_rgba(139,92,246,0.25)]">
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-display font-semibold text-white group-hover:text-gradient transition-all duration-300">
            {item.institution}
          </h3>
          {item.startYear && (
            <span className="shrink-0 rounded-full border border-white/10 px-2 py-0.5 text-xs text-zinc-400 group-hover:border-gold-300/30 group-hover:text-gold-200 transition-all">
              {dateLabel(item)}
            </span>
          )}
        </div>
        <p className="mt-1.5 text-sm font-medium text-gold-300 group-hover:text-gold-200 transition-colors">
          {item.degree}
          ...
          <ul className="mt-3 space-y-1.5 text-sm leading-relaxed text-zinc-400">
            {(item.details ?? []).map((d, j) => (
              <li key={j} className="flex gap-2 group-hover:translate-x-0.5 transition-transform duration-300">
                <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-gold-300/60 group-hover:bg-gold-300 transition-colors" />
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="relative mx-auto max-w-6xl px-6 py-28">
      <div className="pointer-events-none absolute right-0 bottom-0 h-[300px] w-[300px] rounded-full bg-gold-600/10 blur-[100px]" />
            ...
            <div className="relative space-y-8 border-l-2 border-white/10 pl-7">
              ...
              <div ref={lineRef} className="absolute left-[-2px] top-0 h-full w-0.5 bg-white/10">
                <motion.div
                  ...
                  className="w-full bg-gradient-to-b from-gold-500 via-gold-400 to-gold-500 shadow-[0_0_12px_rgba(212,175,55,0.6)]"
                />
              </div>
              {experience.map((x, i) => (
